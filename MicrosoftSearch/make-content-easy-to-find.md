...
> [!Note]
> Non è possibile importare gli&domande e risposte se sono presenti errori nel file modello. Per evitare errori, verificare che il file di importazione sia formattato correttamente e che includa tutte le informazioni necessarie. Per ulteriori informazioni su come evitare errori [di importazione,](#prevent-import-errors)vedere .
...
- Ottenere informazioni necessarie su base ricorrente, in base a cicli di lavoro o a livello di attività. Un esempio è la ricerca di strumenti per la prenotazione di aggiornamenti finanziari trimestrali o di tempo libero.
...
- Se includi *l'ID* di un segnalibro esistente, questo verrà sostituito con le informazioni nel file di importazione.
...
> [!Note]
> Non è possibile importare gli elementi del componente se sono presenti errori nel file modello. Per evitare errori, verificare che il file di importazione sia formattato correttamente e che includa tutte le informazioni necessarie.
...
- **Le impostazioni** di gruppo rendono i risultati disponibili solo per i membri dei gruppi selezionati. Ad esempio, se si creano siti correlati solo ai dipendenti del reparto Risorse umane, eseguire il mapping di questa impostazione al gruppo di sicurezza delle risorse umane appropriato.
...
Le informazioni importanti come [Segnalibri](manage-bookmarks.md) e [Domande&A](manage-qas.md) devono rimanere aggiornate. Eseguire questi passaggi regolarmente:
...
- Valutare se il segnalibro o la&A è sufficientemente autorevole o necessita di una descrizione più chiara.
...
Segnalibri [suggeriti,](manage-bookmarks.md) [Domande&A](manage-qas.md) [](manage-locations.md) e Posizioni sono un buon indicatore delle lacune nel contenuto. Consentono di comprendere ciò che gli utenti ricercano ma non trovano. Questi dati potrebbero indicare che è necessario creare più segnalibri, domande&A o Posizioni. In caso contrario, potrebbe essere necessario aggiornare il contenuto esistente utilizzando parole chiave, parole chiave riservate e stringhe di ricerca migliori per rendere il contenuto più individuabile.
